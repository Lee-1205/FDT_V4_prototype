...
def train(args: argparse.Namespace) -> int:
    ...
    try:
        while tokens_seen - start_tokens < target_tokens:
            ...
            if not bool(torch.stack((torch.isfinite(base_grad_norm), torch.isfinite(exact_grad_norm))).all()):
            ...
            if step % log_every == 0:
                ...
                session_elapsed = max(time.perf_counter() - started, 1e-9)
                tps = session_tokens / session_elapsed
                append_jsonl(log_path, {"event": "train", "step": step, "tokens_seen": tokens_seen, "additional_tokens": additional, "target_additional_tokens": target_tokens, "effective_tokens": batch_tokens, "base_forward_groups": len(forward_groups), "effective_samples_per_step": accumulation, "loss": total_loss, "finite_loss": math.isfinite(total_loss), "entropy_normalized": entropy_value, "dead_anchor_fraction": dead_anchor_value, "top1_membership": top1_membership_value, "tokens_per_sec": tps, "throughput_contract": "session_local_v2", "session_tokens": session_tokens, "eta_seconds": max(target_tokens - additional, 0) / max(tps, 1e-9), "objective_losses": objective_values, "diagnostic_metrics": diagnostic_values, "base_grad_norm": float(base_grad_norm.detach().cpu()), "exact_pointer_grad_norm": float(exact_grad_norm.detach().cpu()), "free_gib": disk_free_gib()})
            if validation_pool is not None and validation_every > 0 and step % validation_every == 0:
                ...
                source_states["__validation__"] = validation_pool.state()
                append_jsonl(log_path, {"event": "validation", "step": step, "validation_loss": validation_loss, "finite_loss": math.isfinite(validation_loss), "validation_batches": int(train_cfg.get("validation_batches", 8)), "validation_contract": gate_version})
                ...
                overfit_triggered = consecutive_regression and train_improved
                settings.update({
